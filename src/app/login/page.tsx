...
const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(json?.error || "Falha ao realizar login. Verifique seu e-mail e senha.");
        setLoading(false);
        return;
      }

      // Atualiza o estado global de autenticação imediatamente
      await refreshUser();
      router.push("/");
      router.refresh();
    } catch (err: any) {
          ...
          <div className="surface-glass card-glow rounded-2xl border border-border p-8 md:p-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <Button
                type="submit"
                disabled={loading}
                className="w-full h-12 text-base font-semibold gold-gradient hover:opacity-90 transition-opacity flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
